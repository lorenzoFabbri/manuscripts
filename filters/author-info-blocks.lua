...
local default_marks
local default_marks = {
    corresponding_author = FORMAT == 'latex' and
        {pandoc.RawInline('latex', '*')} or {pandoc.Str '✉'},
    equal_contributor = FORMAT == 'latex' and
        {pandoc.RawInline('latex', '$\\dagger{}$')} or {pandoc.Str '*'}
}
...
local function intercalate(lists, elem)
    ...
    if #lists > 0 then result:extend(lists[#lists]) end
    return result
end
...
local function author_inline_generator(get_mark)
    return function(author)
        ...
        res[#res + 1] = pandoc.Superscript(
                            intercalate(author_marks, {pandoc.Str ','}))
        return res
    end
end

local function is_equal_contributor(author) return author.equal_contributor end
...
local function create_equal_contributors_block(authors, mark)
    ...
    if not has_equal_contribs then return nil end
    local contributors = {
        pandoc.Superscript(mark 'equal_contributor'), pandoc.Space(),
...
local function create_affiliations_blocks(affiliations)
    local affil_lines = List:new(affiliations):map(function(affil, i)
        local num_inlines = List:new{
            pandoc.Superscript {pandoc.Str(tostring(i))}, pandoc.Space()
        }
...
local function create_correspondence_blocks(authors, mark)
    ...
    for _, author in ipairs(authors) do
        if is_corresponding_author(author) then
            ...
            local author_with_mail = List:new(author.name ..
                                                  List:new{
    ...
    if #corresponding_authors == 0 then return nil end
    local correspondence = List:new{
        pandoc.Superscript(mark 'corresponding_author'), pandoc.Space(),
        pandoc.Str 'Correspondence:', pandoc.Space()
    }
...
local function create_authors_inlines(authors, mark)
    ...
    if #authors > 1 then result:extend(List:new{pandoc.Str ","} .. and_str) end
...
return {
    {
        Pandoc = function(doc)
            ...
            body:extend(
                create_equal_contributors_block(doc.meta.author, mark) or {})
            ...
            meta.author = FORMAT:match 'latex' and
                              pandoc.MetaList(doc.meta.author):map(
                                  author_inline_generator(mark)) or
                              pandoc.MetaInlines(
