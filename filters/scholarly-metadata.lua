...
local function has_id(id) return function(x) return x.id == id end end
...
function add_missing_entries(a, b)
    for k, v in pairs(a) do b[k] = b[k] or v end
    return b
end
    ...
    elseif next(obj) and next(obj, next(obj)) == nil then
        ...
        if type(attribs) == 'string' or type(attribs) == 'Inlines' then
            ...
            named.name = named.name or
                             pandoc.MetaInlines {pandoc.Str(tostring(key))}
        end
...
local function resolve_institutes(institute, known_institutes)
    ...
    for i, inst in ipairs(unresolved_institutes) do
        result[i] = known_institutes[tonumber(inst)] or
                        known_institutes:find_if(
                            has_id(pandoc.utils.stringify(inst))) or
...
local function flatten(lists)
    local result = List:new{}
    for _, lst in ipairs(lists) do result:extend(lst) end
    return result
end
...
local function canonicalize(raw_author, raw_institute)
    ...
    for _, author in ipairs(authors) do
        author.institute = resolve_institutes(ensure_list(author.institute),
                                              institutes)
    end
    ...
    -- top-level list.
    local author_insts =
        flatten(authors:map(function(x) return x.institute end))
    for _, inst in ipairs(author_insts) do merge_on_id(institutes, inst) end
...
return {
    {
        Meta = function(meta)
            meta.author, meta.institute =
                canonicalize(meta.author, meta.institute)
            return meta
        end
    }
}
