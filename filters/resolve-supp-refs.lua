...
local function catalog(doc)
    ...
    pandoc.walk_block(pandoc.Div(doc.blocks), {
        ...
        Figure = function(el)
            if currently_in_supp and el.identifier and
                el.identifier:match("^fig:") then
...
local function replace_link(el)
    ...
    if el.target and el.target:match("^#") then
        ...
        if supp_figs[target_id] then
            return {
                pandoc.Str("Figure"), pandoc.Space(),
                pandoc.Str("S" .. supp_figs[target_id])
            }
        end
        ...
        if supp_tbls[target_id] then
            return {
                pandoc.Str("Table"), pandoc.Space(),
                pandoc.Str("S" .. supp_tbls[target_id])
            }
        end
    end
